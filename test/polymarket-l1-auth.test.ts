...
//
// Locks down the ERC-7739 TypedDataSign wrapping in l1-auth-1271.ts — the
// workaround for clob-client-v2 issue #65. Two layers of defense:
...
import { buildWrapped1271Headers } from "../src/utils/polymarket/l1-auth-1271.js";
...
const DEPOSIT = "0x1111111111111111111111111111111111111111" as const;
const TS = 1750000000;
...
test("wrapped L1 headers match the pinned golden vector", async () => {
...
test("envelope layout: sig ‖ domainSep ‖ contentsHash ‖ typeString ‖ uint16 length", async () => {
  const account = privateKeyToAccount(TEST_KEY);
  const h = await buildWrapped1271Headers(account, DEPOSIT, TS);
...
test("inner signature recovers to the EOA over the TypedDataSign struct", async () => {
  const account = privateKeyToAccount(TEST_KEY);
  const h = await buildWrapped1271Headers(account, DEPOSIT, TS);
  const innerSig = `0x${h.POLY_SIGNATURE.slice(2, 2 + 130)}` as `0x${string}`;
