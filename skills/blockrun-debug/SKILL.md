...
| `blockrun_surf` → `blockrun_surf is retired — the gateway answers every /v1/surf/* path … with 410 endpoint_retired since 2026-09-06` (builds before 0.49.0: `API error: 410 — API request failed`, after a spend-confirmation prompt) | Surf (asksurf.ai) was retired from the gateway on 2026-09-06. No 402 is ever issued. The tool now answers before reserving budget or consulting the wallet. Not an outage. **Not charged.** | Do not retry. Prices → `blockrun_price` (free); TVL/yields → `blockrun_defi`; prediction markets → `blockrun_markets`; DEX pairs → `blockrun_dex` (free). On-chain SQL, wallet labels and social have no BlockRun replacement yet — the `surf` skill lists what moved where. |
