...
interface ClobMarketToken { token_id?: string; outcome?: string }
...
async function resolveToken(
  ...
  if (!match?.token_id) {
    const available = tokens.map((t) => t.outcome).filter(Boolean).join(", ");
    throw new Error(
...
function isCredsMismatchError(err: { message?: string; status?: number; data?: unknown }): boolean {
  ...
  const dataText = typeof err.data === "string" ? err.data.toLowerCase()
    : err.data ? JSON.stringify(err.data).toLowerCase() : "";
...
export async function mapClobError(err: unknown): Promise<string> {
  ...
  if (m.includes("invalid price") || m.includes("tick")) {
    return `Price rejected (${message}). Prices are probabilities on the market's tick grid — ` +
  ...
  if (m.includes("closed") || m.includes("resolved") || m.includes("not accepting")) {
    return `Market is not accepting orders (${message}). If it resolved in your favor, use action:"positions" ` +
      `then action:"redeem".`;
  }
  if (m.includes("fok") || m.includes("killed") || m.includes("not filled")) {
    return `Fill-or-kill order could not fill completely (${message}). Try order_type:"FAK" (fills what it can) ` +
...
export async function executeTrade(input: TradeInput): Promise<ToolResult> {
  ...
  if (!isLimit && input.action === "buy" && input.amount_usd === undefined) {
    return { text: `Market buys need amount_usd (pUSD dollars to spend). For a limit order pass price + size.`, isError: true };
  }
  ...
  if (input.order_type === "GTD" && input.expires_at === undefined) {
    return { text: `GTD orders need expires_at (unix seconds, at least ~3 minutes in the future).`, isError: true };
  }
  if (isLimit && (input.order_type === "FOK" || input.order_type === "FAK")) {
    return { text: `FOK/FAK are market-order types — omit price for a market order, or use GTC/GTD for limits.`, isError: true };
  }
  if (!isLimit && (input.order_type === "GTC" || input.order_type === "GTD")) {
    return { text: `GTC/GTD need a limit price. Pass price + size, or use FOK/FAK for market orders.`, isError: true };
  }
  ...
  try {
    return await withCredsRetry(async (clob) => {
      ...
      if (token.closed || token.acceptingOrders === false) {
        return {
          text: `Market "${token.question ?? token.conditionId}" is not accepting orders (closed/resolved). ` +
      ...
      const price = input.price !== undefined ? roundToTick(input.price, tickSize, input.action) : undefined;
      const tick = parseFloat(tickSize);
      if (price !== undefined && (price < tick || price > 1 - tick)) {
        return {
          text: `Price ${input.price} rounds to ${price}, outside this market's valid range ` +
      ...
      if (notional > maxBet) {
        return {
          text: `Order notional ~$${notional.toFixed(2)} exceeds the per-order cap of $${maxBet} ` +
      ...
      if (sessionCap !== null && ledger.totalUsd + notional > sessionCap) {
        return {
          text: `Session betting total would reach $${(ledger.totalUsd + notional).toFixed(2)}, over the ` +
...
export async function listOpenOrders(input: { condition_id?: string }): Promise<ToolResult> {
  try {
    return await withCredsRetry(async (clob) => {
      const orders = await clob.getOpenOrders(input.condition_id ? { market: input.condition_id } : undefined);
      if (!orders?.length) {
...
export async function cancelOrdersAction(input: { order_id?: string; all?: boolean }): Promise<ToolResult> {
  if (!input.order_id && input.all !== true) {
    return { text: `Pass order_id:"…" or all:true.`, isError: true };
  }
  try {
    return await withCredsRetry(async (clob) => {
      const result = (input.all === true
        ? await clob.cancelAll()
        : await clob.cancelOrder({ orderID: input.order_id as string })) as {
        canceled?: string[];
      ...
      if (failedIds.length) {
        ...
        return {
          text: `⚠️ Cancelled ${canceled.length} order(s); ${failedIds.length} could NOT be cancelled ` +
