...
export async function confirmSpend(
  ...
  try {
    ...
    // Only latch session auto-approve on an explicit accept. Per the elicitation
    // spec `content` is populated only on accept, but gating defensively means a
    // misbehaving client that returns decline+content can't silently auto-approve
    // the rest of the session's charges.
    if (result.action === "accept" && content?.approve_all_session) sessionAutoApprove = true;
